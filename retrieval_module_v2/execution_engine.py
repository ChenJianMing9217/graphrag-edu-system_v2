...
class ExecutionEngine:
    ...
    def _fetch_with_web_search(self, client, query: str, chat_history: List[Dict]) -> List[CandidateNode]:
        ...
        system_prompt = (
            ...
            "\n\n【絕對準確性要求】"
            "\n* 每一筆機構名稱、地址、電話、網址、補助金額、申請期限**都必須來自實際 web_search 搜到的網頁**，"
            "嚴禁從訓練記憶或推測中編造任何細節。"
            "\n* 若搜尋結果未涵蓋使用者問的縣市或具體資訊，請**明確說明「目前查無 XX 縣市的官方資料，建議直接致電該縣市社會局」**，"
            "不可用其他縣市的資料替代或杜撰。"
            "\n* 任何條目都必須附上**完整可點擊的 URL**作為來源；找不到來源就不要列那一條。"
            "\n* 若你對某項細節（如金額、期限、聯絡電話）不完全確定，請寫「（請以最新公告為準，建議直接查詢…）」而非寫具體數字。"
            "\n* 補助金額/條件請註明「以最新公告為準」並附上公告 URL。"
            "\n\n【格式規則】"
            "\n1. 優先引用官方來源（衛福部、社家署、各縣市社會局、教育部）。"
            "\n2. 機構或補助條目以條列式呈現，每條至少含：名稱、來源網址。"
            "\n3. 全程使用繁體中文與台灣用語。"
            "\n4. 回答限 400 字以內。"
            "\n5. **若沒有可信來源，回覆「目前未查到可靠資料」即可，不要硬擠內容。**"
        )
    ...
    def _fetch_chat_completion(self, client, query: str, chat_history: List[Dict]) -> List[CandidateNode]:
        ...
        try:
            system_prompt = (
                ...
                "\n\n【絕對準確性要求】"
                "\n1. 此模式**沒有上網查詢**，不要列任何具體機構名稱、地址、電話、網址、補助金額。"
                "\n2. 涉及在地資源時，請說「請至衛福部社家署官網（https://www.sfaa.gov.tw）或各縣市社會局查詢最新資訊」。"
                "\n3. 補助條件、申請期限、金額**僅給通用方向**，具體數字請寫「以該縣市最新公告為準」。"
                "\n4. **若你對某項知識不確定（例如最新法規、特定診斷標準的細節），請直接回覆「此項建議諮詢專業臨床或最新官方公告」，"
                "不要編造數字或來源**。"
                "\n5. 若引用任何專業指引（如 DSM-5、ICF），請註明出處名稱（不需要 URL，但名稱要正確）。"
                "\n6. 全程繁體中文 + 台灣用語。"
            )
